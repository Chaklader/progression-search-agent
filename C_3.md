...
flowchart TD
    ...
    C --> D["Rule: Farmer(f) & Rabbit(r) ==> Hates(f, r)"]
    D --> E["Unify with Facts"]
    E --> F["Fact: Farmer(Mac)"]
    E --> G["Fact: Rabbit(Flopsie)"]
    F --> H["Substitution: {f: Mac, r: Flopsie}"]
    G --> H
...
```mermaid
flowchart TD
    A["Original Expression<br>(A ==> B) & (~B | C)"] --> B["Eliminate Implications<br>(~A | B) & (~B | C)"]
    B --> C["Move Negations Inward<br>(~A | B) & (~B | C)"]
    C --> D["Distribute AND over OR<br>(~A | B) & (~B | C)"]
    D --> E["Final CNF Form<br>Same as input in this case"]

    F["Expression Tree"] --> G["Operator: &"]
    G --> H["Left: (~A | B)"]
    G --> I["Right: (~B | C)"]
    H --> J["Operator: |"]
    J --> K["~A"]
...
```mermaid
flowchart TD
    A["Original Expression<br>(A & B) | (C & D)"] --> B["Apply Distribution<br>(A | C) & (A | D) & (B | C) & (B | D)"]
    ...
    F --> G["3. Distribute AND over OR<br>(A & B) | C becomes (A | C) & (B | C)"]
