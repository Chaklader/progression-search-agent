...
You've raised an excellent point about the realism of our logical statement. Let's examine how to express "some dogs
have human owners" rather than "all dogs have human owners."
...
sets. The stochastic robot movement action you've provided represents a non-deterministic action where the robot's
attempt to move might have different possible outcomes. Let me format this properly using LaTeX notation:
...
`Move(robot, from, to)` names the action and specifies three parameters:
...
Before the robot can attempt this move, two conditions must be true:

1. `At(robot, from)`: The robot must currently be at the starting location
2. `Adjacent(from, to)`: The starting and intended destination locations must be adjacent

###### Effects

This is where things get interesting. The effects are listed as three possibilities connected by the logical OR operator
(∨):

1. **First possibility**: `[At(robot, to) ∧ ¬At(robot, from)]`
...
2. **Second possibility**: `[At(robot, from)]`
...
3. **Third possibility**: `[At(robot, adjacent-unintended) ∧ ¬At(robot, from)]`
