...
"For all dirt $d$ and locations $l$, if d is dirt and $l$ is a location, then $d$ is not at $l$." (Initially, no dirt is
at any location)
...
vacuum cleaner, $V$ is at location $l$, and the dirt $d$ is also at location $l$."
...
- O is a set of ordering constraints ($a_i \lt a_j$)
- L is a set of causal links ($a_i →[p]→ a_j$)
...
3. Add a causal link $a_i →[p]→ a_j$
4. Add ordering constraint $a_i \lt a_j$
...
Let me create a diagram that illustrates the structure and process of And-Or search trees for generating contingency
plans in nondeterministic domains:
...
flowchart TD
    ...
    style BS fill:#FA756A
    style PL fill:#9AE4F5
    style MC fill:#0096D9
    style S1 fill:#BCFB89
    style S2 fill:#BCFB89
    style S3 fill:#BCFB89
    style ES1 fill:#FBF266
    style ES2 fill:#FBF266
    style ES3 fill:#FBF266
    style AS fill:#FCEB14
    style A1 fill:#FE9237
    style EX fill:#FE9237
    style OB fill:#BCFB89
    style UB fill:#FA756A
    style PL2 fill:#9AE4F5
```
