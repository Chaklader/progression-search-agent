...
- Time complexity: $O(b^{1 + \lfloor C^*/\epsilon \rfloor})$ where $C^*$ is the cost of the optimal solution and ε is the minimum action cost
...
1. **Admissibility**: A heuristic function $h(n) $ is admissible if it never overestimates the true cost to reach the goal from any node $n $. Mathematically, for all nodes $n $: $h(n) \leq h^*(n)$ where $h^*(n) $ is the true optimal cost from node $n $ to the goal state.
